...
from app.detection.types import ApprovedMapping, DetectionResult
...
class MappingIndex:
    def __init__(
        ...
        # overrides kept for backward-compat with existing tests — ignored
        ...
        # Index: (keyword_lower, circuit_lower) → screen_format for circuit-specific mappings
        self._circuit_index: dict[tuple[str, str], str] = {
            (m.amenity_keyword.lower(), m.circuit_name.lower()): m.screen_format
            for m in self.mappings
            if m.circuit_name
        }
        # Keywords that have at least one circuit-specific mapping
        self._has_circuit_mapping: frozenset[str] = frozenset(
            m.amenity_keyword.lower() for m in self.mappings if m.circuit_name
        )
...
class ScreenFormatEngine:
    ...
    def _match_segment(
        ...
        # Track C: token + concat corruption matching
        try:
    ...
    def _resolve_circuit_format(self, mapping: ApprovedMapping, norm_circuit_lower: str) -> str:
        # Check circuit-specific mapping in index first
        if norm_circuit_lower:
            key = (mapping.amenity_keyword.lower(), norm_circuit_lower)
            if key in self.index._circuit_index:
                return self.index._circuit_index[key]

        if not mapping.circuit_name:
            return mapping.screen_format

        # Mapping is circuit-scoped but circuit doesn't match
    ...
    def detect(self, amenity: str, circuit_name: str = "") -> DetectionResult:
        ...
        best_hit: Optional[tuple[ApprovedMapping, str, str, int]] = None
            ...
            if best_hit is None:
                ...
                if mapping.priority_tier < prev_mapping.priority_tier:
                    ...
                    new_circuit_match = (
                        ...
                        and mapping.circuit_name.lower() == norm_circuit_lower
                    )
                    prev_circuit_match = (
                        prev_mapping.circuit_name is not None
                        and prev_mapping.circuit_name.lower() == norm_circuit_lower
                    )
