...
from app.models import MovieFormatJob

router = APIRouter(prefix="/api/v1/movie-jobs", tags=["movie-jobs"])
...
def get_job(job_id: str, session: Session = Depends(get_session)) -> dict:
    ...
    return {
        ...
        "output_url": f"/api/v1/movie-jobs/{job.id}/download" if job.status == "completed" and job.output_path else None,
    }
...
def download_job(job_id: str, session: Session = Depends(get_session)) -> FileResponse:
    ...
    return FileResponse(
        ...
        filename=f"movie_format_results_{job_id[:8]}.xlsx",
    )
