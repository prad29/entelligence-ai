...
import { useIntlBatchJob } from '@/hooks/useIntlBatchJob'
...
function IntlBatchUploader() {
  const [file, setFile] = useState<File | null>(null)
  const [includeDiagnostics, setIncludeDiagnostics] = useState(false)
  const [auditMode, setAuditMode] = useState(false)
  const { job, uploading, isActive, resuming, error, uploadIntlBatch, reset } = useIntlBatchJob()
  ...
  const handleUpload = async () => {
    if (!file) return
    await uploadIntlBatch(file, includeDiagnostics, auditMode)
  }
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Upload a CSV or XLSX file to detect international screen formats in bulk</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
              ...
              {file ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs text-zinc-400 dark:text-zinc-500">
                    CSV or XLSX files supported — must include an <code className="font-mono">amenities</code> column
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {auditMode && (
                <div className="ml-12 flex flex-col gap-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                  <p>File must include: <code className="font-mono">amenities</code>, <code className="font-mono">screen_format</code></p>
                  <p>Output adds: <code className="font-mono">screen_format</code>, <code className="font-mono">match_track</code>, <code className="font-mono">match_source</code>, <code className="font-mono">match_status</code></p>
                </div>
              )}
            </div>
          )}
          ...
          {job && (
            <div className="flex flex-col gap-4">
              ...
              {/* Stats: Total / Keyword Match / No Match only — no AI, no anomaly detection for intl */}
