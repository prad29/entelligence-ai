...
import { useIntlDetect } from '@/hooks/useIntlDetect'
import { Button } from '@/components/ui/Button'
import { Textarea } from '@/components/ui/Textarea'
import { IntlResultCard } from './IntlResultCard'
...
function IntlSingleDetector() {
  const [amenityString, setAmenityString] = useState('')
  const { result, loading, error, detect, reset } = useIntlDetect()
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <Textarea
            ...
            id="intl-amenity-string"
            value={amenityString}
            onChange={(e) => {
              setAmenityString(e.target.value)
              if (result) reset()
            }}
            placeholder="e.g. 4DX | ScreenX | Dolby Cinema"
            rows={3}
          />
      ...
      {result && <IntlResultCard result={result} />}
    </div>
  )
}

export { IntlSingleDetector }
