...
import { useMovieFormats, type MovieFormat } from '@/hooks/useMovieFormats'
import { DataTable, type Column } from '@/components/ui/DataTable'
import { Badge, type BadgeVariant } from '@/components/ui/Badge'
import { Button } from '@/components/ui/Button'
import { Select } from '@/components/ui/Select'
import { MovieFormatFormDrawer } from './MovieFormatFormDrawer'
import { Plus, Pencil, Check, X, Download, Search, ChevronLeft, ChevronRight } from 'lucide-react'
...
const tierOptions = [
  ...
  ...['P1', 'P2', 'P3', 'P4'].map((t) => ({ value: t, label: t })),
]
...
function MovieFormatsPage() {
  ...
  const [editTarget, setEditTarget] = useState<MovieFormat | null>(null)

  const { formats, total, totalPages, loading, createFormat, updateFormat, deleteFormat } = useMovieFormats({
    search,
  ...
  const columns: Column<MovieFormat>[] = [
    {
      key: 'keyword',
      header: 'Keyword',
      sortable: true,
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => (
        <span className="font-medium text-zinc-800 dark:text-zinc-200">{row.format}</span>
      ),
    },
    {
      key: 'tier',
          ...
          {(row.status === 'pending' || row.status === 'draft') && (
            <>
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  void updateFormat(row.id, { status: 'approved' })
                }}
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  void updateFormat(row.id, { status: 'inactive' })
                }}
  ...
  const handleSubmit = async (data: Omit<MovieFormat, 'id' | 'updated_at'>) => {
    if (editTarget) {
      await updateFormat(editTarget.id, data)
    } else {
  ...
  const handleExport = () => {
    ...
    window.open(`${base}/api/v1/movie-formats/export`, '_blank')
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          value={formatFilter}
          onValueChange={(v) => { setFormatFilter(v); resetPage() }}
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No movie format mappings found. Add one to get started."
        />
      )}
