...
function ReviewTable({ items, onApprove, onReject }: {
  ...
  const columns: Column<MovieReviewItem>[] = [
    {
      key: 'type',
      header: 'Type',
      cell: (row) => (
...
function MovieReviewQueuePage() {
  const [items, setItems] = useState<MovieReviewItem[]>([])
  const [loading, setLoading] = useState(true)

  const fetchItems = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get<MovieReviewItem[]>('/api/v1/movie-review?limit=200')
      setItems(res.data)
    } finally {
      setLoading(false)
    }
